...
u = fi( [linspace(-5,20,1000)] );
y = fi_exp2lookup_8_bit_byte(u);
y_expected = 2.^(double(u));
...
function y = fi_exp2lookup_8_bit_byte(u)
    % Load the lookup table
    EXP2LUT = exp2_lookup_table();
    % Remove fimath from the input to insulate this function from math
    % settings declared outside this function.
    u = removefimath(u);
    % Declare the output
    y = coder.nullcopy(fi(zeros(size(u)),numerictype(EXP2LUT),fimath(EXP2LUT)));
    ...
    for k = 1:numel(u)
        ...
        shift_1 = bitsliceget(u(k), w, w - 6 + 1);
        shift_1 = fi(shift_1, 1);
        shift = storedInteger(shift_1);
      
        temp = bitsliceget(u(k), w - 6, w-6-6);
        id = storedInteger(bitsliceget(u(k), w - 6, w-6-6));
...
function EXP2LUT = exp2_lookup_table()
    B = 8;  % Number of bits in a byte
    exp2_table = 2.^(0:(1/2^(B-1)):(1-1/(2^(B-1))));
    ...
    log2_table = [0.000000000000000   0.011227255423254   0.022367813028454   0.033423001537450 ...
     ...
     WL = 4*B;  % Word length
     FL = 2*B - 1;  % Fraction length
     EXP2LUT = fi(exp2_table,1,WL,FL,'RoundingMethod','Nearest');
